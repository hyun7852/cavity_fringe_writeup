...
if __name__ == '__main__':
	...
	N=5000
	f=np.linspace(-500,500,N) #frequency of laser or detuning
	intenstiy=lorenz(f,0,200)

	# Omega_array=lorenz(mu=Omega,sigma=max_omega,N=N)
	...
	for i, detuning in enumerate(f):

		if i ==0:
			dynamics=excited_state_fraction(t,Omega*intenstiy[i],2*np.pi*detuning)
		else:
			temp=excited_state_fraction(t,Omega*intenstiy[i],2*np.pi*detuning)
			dynamics+=temp

	# on_res=excited_state_fraction(t,Omega,0)
	# detuned=excited_state_fraction(t,Omega,2*np.pi*400)
	dynamics=dynamics/N
	ax.plot(t*1e3,dynamics,":o",label=r"$\Omega_{\sigma}$= 2*pi*"+str(100) + " Hz")
