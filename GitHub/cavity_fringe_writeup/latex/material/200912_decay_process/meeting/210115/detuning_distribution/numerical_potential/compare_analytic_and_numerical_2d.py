...
def intensity_profile(xx,yy,I0):
    ...
    r2=(xx**2+yy**2)
    return I0*(np.exp(-2*(xx**2/CAVITY_WAIST**2))+(np.exp(-2*(yy**2/CAVITY_WAIST**2))))
...
def get_lattice_detuning(xx,yy,v_trap=106800,norm=True):
    ...
    ac_stark_2d=ac_stark
    if norm==True:
        print("max_ac",np.max(ac_stark_2d))        
...
if __name__ == '__main__':
    ...
    for j,numb_pix in enumerate(numb_pix_array):
        ...
        # plt.imshow(Detuning_array/2/np.pi)
        # plt.show()
    ...
    fig.savefig('excfraction_VS_time_potent_center.pdf')
